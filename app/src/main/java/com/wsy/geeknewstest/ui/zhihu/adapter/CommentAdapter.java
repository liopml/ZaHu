...
import com.wsy.geeknewstest.widget.CircleImageView;
...
//            holder.tvReply.setText(String.format("@%s: %s",info.getReply_to().getAuthor(),info.getReply_to().getContent()));
            ...
            if (info.getReply_to().getExpandState() == STATE_NULL) {    //未知
                holder.tvReply.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
                    @Override
                    public boolean onPreDraw() {
                        //这个回调会调用多次，获取完行数记得注销监听
                        holder.tvReply.getViewTreeObserver().removeOnPreDrawListener(this);
                        //记录文本的状态
